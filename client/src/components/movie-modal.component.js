...
export default function MovieModal(props) {
  ...
  const handleFavourite = () => {
    if (isFavouriteMovieFound) {
      return <Button onClick={() => removeFavourite()}>Remove from Favourites</Button>;
    } else {
  ...
  return (
    <Modal
      ...
      <Modal.Header closeButton>
        <Modal.Title id="contained-modal-title-vcenter">{props.movie.title}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <div className="row">
          <div className="col-sm-12 col-md-4">
            <img
              className="moviePoster movie-card modal-img"
              key={props.movie.key}
              src={
        ...
        <form onSubmit={onSubmit} className="form-review">
          ...
          <button
            className="btn btn-lg btn-primary btn-block text-uppercase review-btn"
            type="submit"
            disabled={userReview <= 0}>
            Post
          </button>
        </form>
        {reviews.map((currentReview) => (
          <Reviews review={currentReview} key={currentReview._id} movie={props.movie} />
        ))}
      </Modal.Body>
      <Modal.Footer>
        {handleFavourite()}
