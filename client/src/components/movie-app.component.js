...
export default function MovieApp() {
  ...
  const Movies = (props) => (
    <>
      <button type="button" className="movie-poster-button" onClick={() => imageClick(props.movie)}>
        <img
          className="moviePoster movie-card"
          key={props.movie.key}
          src={
            props.movie.poster_path
              ? "https://image.tmdb.org/t/p/original" + props.movie.poster_path
              : require("../Assets/no_poster.jpg")
          }
          width="200px"
          height="300px"
      ...
      <form onSubmit={onSubmit} className="form-add-task">
          ...
          <div className="col-sm-12 d-flex">
            {[DropdownButton].map((DropdownType, idx) => (
              <DropdownType
                ...
                <Dropdown.Item eventKey="1" onClick={() => readPopularMovies(1)}>
                  Get Most Popular
                </Dropdown.Item>
                <Dropdown.Item eventKey="2" onClick={() => readNowPlayingMovies(1)}>
                  Get Now Playing in Theatres
                </Dropdown.Item>
                <Dropdown.Item eventKey="3" onClick={() => readUpcomingMovies(1)}>
                  Get Upcoming Movies
                </Dropdown.Item>
              </DropdownType>
            ))}
        ...
        <div className="container card-container">
          {movies.map((currentMovie) => (
            <Movies movie={currentMovie} key={currentMovie.id} />
          ))}
